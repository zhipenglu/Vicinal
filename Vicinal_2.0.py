...
for line in softclipreads:
    ...
    if currentchr != record[2]:
        if currentchr != '':   #write to disk reads that are mapped to the current chromosome
            ...
            chrposv1dict = {i:chrposv1.count(i) for i in chrposv1}
            chrposv2dict = {i:chrposv2.count(i) for i in chrposv2}
            for pos in sorted(chrposv1dict.keys()):
                #if chrposv1dict[pos] == 0: continue
                wig1.write(str(pos) + '\t' + str(chrposv1dict[pos]) + '\n')
            for pos in sorted(chrposv2dict.keys()):
                #if chrposv2dict[pos] == 0: continue
                wig2.write(str(pos) + '\t' + str(-chrposv2dict[pos]) + '\n')
        currentchr = record[2] #keep track of chrname
        chrseq = getchrseq(currentchr)  #get chromosome sequence
        chrposv1 = [] #initialize list to store partially mapped read
        chrposv2 = [] #initialize list to store partially mapped read
    strand     = record[1]
    ...
    clip1seq = readseq[0: int(cigars[0][0:-1])+1]                        #clip1 + match + clip2
    matchseq = readseq[int(cigars[0][0:-1]): readlen-int(cigars[-1][0:-1])+1] #existence is determined later          #
    clip2seq = readseq[readlen-int(cigars[-1][0:-1]):readlen+1]          #
