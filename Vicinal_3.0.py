# Vicinal v3.0
# This version is not a compatible with v1.0 and v2.0 which
# search for chimeric reads derived from opposite strands.
# Vicinal v3.0 searches for reads on the same strand mapped in chiastic manner, which
# suggest circular RNAs.
# Author: Zhipeng Lu
...
trackdef1 = 'track type=wiggle_0 name="' + label +\
           '_1" visibility=full autoScale=off viewLimits=0:500\n'
trackdef2 = 'track type=wiggle_0 name="' + label +\
           '_2" visibility=full autoScale=off viewLimits=-500:0\n'
...
for line in softclipreads:
    ...
    readseq    = record[9]
    refseq1 = chrseq[matchstart-radius: matchstart+readlen]  #search the left side
    refseq2 = chrseq[matchstart: matchstart+readlen+radius]  #search the right side
    ...
    if strand == '0':
        if (cigars[0][-1] == 'S') and (cigars[-1][-1] == 'S') and (int(cigars[0][0:-1]) == int(cigars[0][0:-1])):
            a = [m.start() for m in re.finditer(clip1seq, refseq2)]        
            b = [m.start() for m in re.finditer(clip2seq, refseq1)]
